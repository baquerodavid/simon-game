...
//7.1.b. You'll need a way to keep track of whether if the game has started or not, so you only call nextSequence() on the first keypress.
var started = false;

//7.2. Create a new variable called level and start at level 0.
var level = 0;

//7.1.a. Use jQuery to detect when a keyboard key has been pressed, when that happens for the first time, call nextSequence().
...
$(".btn").click(function() {
    ...
    //4.4 At this stage, if you log the userClickedPattern you should be able to build up an array in the console by clicking on different buttons
...
function playSound(name) {
    ...
    //3.3. Use Google/Stackoverflow to figure out how you can use Javascript to play the sound for the button colour selected in step 1. (randomChosenColour instead of name)
